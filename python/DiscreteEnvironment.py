...
class DiscreteEnvironment:
    ...
    def __init__(self, project_folder):       
        ...
        self.observation_space = DiscreteObservationSpace(3)

        
    def reset(self):
        self.observation = []
        self.original_path = random.choice(self.list_paths)
        self.observation = [self.original_path[0][0], self.original_path[0][0], self.original_path[-1][1]]
        self.generated_path = [self.original_path[0][0]]
        return self.observation
    
    def step(self, action):
        next_edge = list(self.G.edges(self.observation[0]))[action] #we transform the action in a chosen edge
        done = False

        if(next_edge[1] == self.observation[-1]): #if the next node is the final node, high reward because the agent found the destination in the graph
            reward = +10
            done = True
        ...
        if(len(self.generated_path)>1 or self.observation[1] != self.observation[0]):
            self.generated_path.append(self.observation[1])
        self.observation[1] = next_edge[1] #the actual node is now the one at the end of the chosen edge
